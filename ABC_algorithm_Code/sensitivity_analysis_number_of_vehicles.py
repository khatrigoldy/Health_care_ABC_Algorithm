...
def load_problem_data():
    """Load the complete problem data"""
    ...
    # Base capacities
    P = {d: 3000 for d in VD}
    Q = {s: {d: 1000 for d in VD} for s in VS}
    W = {s: 20000 for s in VS}

    return VD, VS, VC, distances, demands, P, Q, W


def create_subset_problem(full_distances, full_demands, VD, VS, selected_customers):
    """Create a problem subset with only selected customers"""
    subset_distances = {}
    all_nodes = VD + VS + selected_customers

    for node1 in all_nodes:
        if node1 in full_distances:
            subset_distances[node1] = {}
            for node2 in all_nodes:
                if node2 in full_distances[node1]:
                    subset_distances[node1][node2] = full_distances[node1][node2]

    subset_demands = {c: full_demands[c] for c in selected_customers if c in full_demands}

    return subset_distances, subset_demands


def run_sensitivity_scenario(problem, params, num_vehicles_config, n_runs=3):
    """Run multiple runs for a specific vehicle configuration"""
    fitness_values = []
    execution_times = []
    successful_runs = 0
    total_vehicles = sum(num_vehicles_config.values())

    print(f"    Testing {total_vehicles} vehicles: {num_vehicles_config}")
    ...
    for run in range(1, n_runs + 1):
        try:
            ...
            abc = MDMSMCVRP_ABC(
                problem=problem,
                num_vehicles_per_satellite=num_vehicles_config,
                swarm_size=params['swarm_size'],
                max_iterations=params['max_iterations'],
                employed_ratio=params['employed_ratio'],
                onlooker_ratio=params['onlooker_ratio'],
                limit=params['limit']
            )
            ...
            if validation['is_valid']:
                ...
                successful_runs += 1
                print(f"      Run {run}: ✅ Fitness = {best_solution.fitness:.2f}")
            else:
                print(f"      Run {run}: ⚠️ Invalid solution")
    ...
    if successful_runs > 0:
        return {
            'num_customers': len(problem.VC),
            'num_vehicles': total_vehicles,
            'vehicle_config': num_vehicles_config,
...
def run_customer_vehicle_sensitivity(VD, VS, all_customers, full_distances, full_demands, P, Q, W, base_params,
                                     n_runs_per_scenario=3):
    """Run sensitivity analysis varying customers and vehicles"""
    print("\n🔬 CUSTOMER-VEHICLE SENSITIVITY ANALYSIS")
    print("=" * 70)

    # Customer subset sizes to test
    customer_counts = [10, 20, 30, 41]

    # Vehicle configurations - optimized for problem scale
    vehicle_configs = [
        {'S1': 2, 'S2': 2},  # 4 vehicles
        {'S1': 3, 'S2': 3},  # 6 vehicles
        {'S1': 4, 'S2': 4},  # 8 vehicles
        {'S1': 5, 'S2': 5},  # 10 vehicles
        {'S1': 6, 'S2': 6},  # 12 vehicles

    ]

    print(f"📊 Customer Counts: {customer_counts}")
    print(f"📊 Vehicle Configurations: {[sum(v.values()) for v in vehicle_configs]} vehicles")

    total_scenarios = len(customer_counts) * len(vehicle_configs)
    print(f"📊 Total Scenarios: {total_scenarios}")
    print(f"📊 Runs per scenario: {n_runs_per_scenario}")
    print(f"📊 Total runs: {total_scenarios * n_runs_per_scenario}")
    print()
    ...
    for num_customers in customer_counts:
        print(f"\n{'=' * 70}")
        print(f"Testing with {num_customers} customers")
        print(f"{'=' * 70}")

        # Select random subset (with fixed seed for reproducibility)
        random.seed(42)
        if num_customers < len(all_customers):
            selected_customers = random.sample(all_customers, num_customers)
        else:
            selected_customers = all_customers

        # Create subset problem
        subset_distances, subset_demands = create_subset_problem(
            full_distances, full_demands, VD, VS, selected_customers
        )

        problem = MDMSMCVRPProblem(
            VD, VS, selected_customers,
            subset_distances, subset_demands,
            P, Q, W
        )

        print(f"Problem created: {len(problem.VC)} customers")
        total_demand = sum(sum(problem.demands[c].values()) for c in problem.VC)
        print(f"Total demand: {total_demand:.1f}")

        # Test each vehicle configuration
        for vehicle_config in vehicle_configs:
            scenario_count += 1
            total_vehicles = sum(vehicle_config.values())
            print(f"\n🎯 Scenario {scenario_count}/{total_scenarios}: "
                  f"{num_customers} customers, {total_vehicles} vehicles")

            result = run_sensitivity_scenario(
                problem, base_params, vehicle_config, n_runs_per_scenario
            )
...
def analyze_customer_vehicle_relationship(results):
    """Analyze relationship between customers and vehicles"""
    print("\n📊 CUSTOMER-VEHICLE RELATIONSHIP ANALYSIS")
    print("=" * 70)

    customer_groups = {}
    for result in results:
        if result['successful_runs'] > 0:
            num_cust = result['num_customers']
            if num_cust not in customer_groups:
                customer_groups[num_cust] = []
            customer_groups[num_cust].append(result)

    for num_cust in sorted(customer_groups.keys()):
        print(f"\n📦 {num_cust} Customers:")
        print(f"{'Vehicles':<12} {'Mean Fitness':<15} {'Best Fitness':<15} {'Success Rate'}")
        print("-" * 60)

        customer_results = customer_groups[num_cust]
        customer_results.sort(key=lambda x: x['mean_fitness'])

        for result in customer_results:
            success_rate = result['success_rate'] * 100
            print(f"{result['num_vehicles']:<12} {result['mean_fitness']:<15.2f} "
                  f"{result['best_fitness']:<15.2f} {success_rate:.0f}%")

        if customer_results:
            optimal = customer_results[0]
            print(f"\n  🏆 Optimal: {optimal['num_vehicles']} vehicles "
                  f"(Fitness: {optimal['mean_fitness']:.2f})")
    ...
    for result in results:
        if result['successful_runs'] > 0:
            row = {
                ...
                'Vehicle_Config': str(result['vehicle_config']),
...
def save_sensitivity_results(results, table_data):
    ...
    detailed_filename = f'customer_vehicle_sensitivity_{timestamp}.json'
    with open(detailed_filename, 'w') as f:
        json.dump({
            'experiment_info': {
                'timestamp': datetime.now().isoformat(),
                'analysis_type': 'customer_vehicle_sensitivity',
                'varied_parameters': ['num_customers', 'num_vehicles'],
                'fixed_parameters': ['primary_capacity', 'satellite_capacity', 'secondary_capacity'],
            },
            'results': results
        }, f, indent=2, default=str)

    table_filename = f'customer_vehicle_table_{timestamp}.json'
    with open(table_filename, 'w') as f:
        json.dump({
            'table_headers': ['Num_Customers', 'Num_Vehicles', 'Vehicle_Config', 'Success_Rate',
...
def create_sensitivity_visualization(results):
    ...
    # 1. Customer count vs Mean Fitness by vehicle config
    ax1 = plt.subplot(2, 3, 1)
    customer_counts = sorted(list(set([r['num_customers'] for r in successful_results])))
    vehicle_counts = sorted(list(set([r['num_vehicles'] for r in successful_results])))

    for num_v in vehicle_counts:
        vehicle_data = [r for r in successful_results if r['num_vehicles'] == num_v]
        vehicle_data.sort(key=lambda x: x['num_customers'])

        x = [r['num_customers'] for r in vehicle_data]
        y = [r['mean_fitness'] for r in vehicle_data]

        ax1.plot(x, y, 'o-', label=f'{num_v} vehicles', markersize=8, linewidth=2)

    ax1.set_xlabel('Number of Customers')
    ax1.set_ylabel('Mean Fitness')
    ax1.set_title('Fitness vs Customer Count by Vehicle Configuration')
    ax1.legend()
    ax1.grid(True, alpha=0.3)

    # 2. Vehicle count vs Mean Fitness by customer count
    ax2 = plt.subplot(2, 3, 2)

    for num_c in customer_counts:
        customer_data = [r for r in successful_results if r['num_customers'] == num_c]
        customer_data.sort(key=lambda x: x['num_vehicles'])

        x = [r['num_vehicles'] for r in customer_data]
        y = [r['mean_fitness'] for r in customer_data]

        ax2.plot(x, y, 's-', label=f'{num_c} customers', markersize=8, linewidth=2)

    ax2.set_xlabel('Number of Vehicles')
    ax2.set_ylabel('Mean Fitness')
    ax2.set_title('Fitness vs Vehicle Count by Customer Count')
    ax2.legend()
    ...
    fitness_matrix = np.zeros((len(vehicle_counts), len(customer_counts)))

    for i, num_v in enumerate(vehicle_counts):
        for j, num_c in enumerate(customer_counts):
            result = next((r for r in successful_results
                           if r['num_vehicles'] == num_v and r['num_customers'] == num_c), None)
            if result:
                fitness_matrix[i, j] = result['mean_fitness']
            else:
                fitness_matrix[i, j] = np.nan

    im = ax3.imshow(fitness_matrix, cmap='viridis', aspect='auto')
    plt.colorbar(im, ax=ax3, label='Mean Fitness')
    ax3.set_xlabel('Number of Customers')
    ax3.set_ylabel('Number of Vehicles')
    ax3.set_title('Fitness Heatmap')
    ax3.set_xticks(range(len(customer_counts)))
    ax3.set_xticklabels(customer_counts)
    ax3.set_yticks(range(len(vehicle_counts)))
    ax3.set_yticklabels(vehicle_counts)

    # 4. Success rate
    ax4 = plt.subplot(2, 3, 4)
    success_by_config = {}
    for result in results:
        key = f"{result['num_customers']}C-{result['num_vehicles']}V"
        success_by_config[key] = result['success_rate'] * 100

    configs = list(success_by_config.keys())
    success_rates = list(success_by_config.values())

    bars = ax4.bar(range(len(configs)), success_rates, alpha=0.7)
    ax4.set_xticks(range(len(configs)))
    ax4.set_xticklabels(configs, rotation=45, ha='right')
    ax4.set_xlabel('Configuration')
    ax4.set_ylabel('Success Rate (%)')
    ax4.set_title('Success Rate by Configuration')
    ax4.grid(True, alpha=0.3)

    for i, (bar, rate) in enumerate(zip(bars, success_rates)):
        bar.set_color('green' if rate == 100 else 'orange' if rate >= 50 else 'red')

    # 5. Execution time
    ax5 = plt.subplot(2, 3, 5)
    exec_times = [r['mean_execution_time'] for r in successful_results]
    num_customers_list = [r['num_customers'] for r in successful_results]

    scatter = ax5.scatter(num_customers_list, exec_times,
                          c=[r['num_vehicles'] for r in successful_results],
                          cmap='plasma', s=100, alpha=0.6)
    plt.colorbar(scatter, ax=ax5, label='Num Vehicles')
    ax5.set_xlabel('Number of Customers')
    ax5.set_ylabel('Execution Time (seconds)')
    ax5.set_title('Execution Time vs Problem Size')
    ax5.grid(True, alpha=0.3)

    # 6. Optimal vehicle count
    # 6. Optimal vehicle count
    ...
    for num_c in customer_counts:
        customer_data = [r for r in successful_results if r['num_customers'] == num_c]
        if customer_data:
            optimal = min(customer_data, key=lambda x: x['mean_fitness'])
    optimal_vehicles[num_c] = optimal['num_vehicles']

    x = list(optimal_vehicles.keys())
    y = list(optimal_vehicles.values())

    ax6.plot(x, y, 'ro-', markersize=12, linewidth=3)
    ax6.set_xlabel('Number of Customers')
    ax6.set_ylabel('Optimal Number of Vehicles')
    ax6.set_title('Optimal Vehicle Count by Customer Count')
    ax6.grid(True, alpha=0.3)

    plt.tight_layout()

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    plot_filename = f'customer_vehicle_sensitivity_{timestamp}.png'
...
def main():
    """Main function for customer-vehicle sensitivity analysis"""
    try:
        print("🔬 MDMS-MCVRP CUSTOMER-VEHICLE SENSITIVITY ANALYSIS")
        print("=" * 70)

        print("📋 Loading problem data...")
        VD, VS, VC, distances, demands, P, Q, W = load_problem_data()

        print(f"Full problem: {len(VD)} depots, {len(VS)} satellites, {len(VC)} customers")

        # Reduced parameters for faster sensitivity testing
        ...
        print(f"Using parameters: {base_hyperparameters}")

        results = run_customer_vehicle_sensitivity(
            VD, VS, VC, distances, demands, P, Q, W,
            base_hyperparameters,
            n_runs_per_scenario=3
        )

        analyze_customer_vehicle_relationship(results)
