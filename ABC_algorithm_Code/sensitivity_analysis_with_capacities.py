...
def run_sensitivity_scenario(problem, params, satellite_cap, secondary_cap, n_runs=5):
    """Run multiple runs for a specific capacity combination with fixed 4 vehicles"""
    fitness_values = []
    execution_times = []
    successful_runs = 0

    # Fixed optimal vehicle configuration: 2 vehicles per satellite (total 4)
    num_vehicles_per_satellite = {'S1': 2, 'S2': 2}
    ...
    print(f"  Testing: Satellite={satellite_cap}, Secondary={secondary_cap}, Vehicles=4 (2 per satellite)")

    for run in range(1, n_runs + 1):
        try:
            start_time = time.time()

            # Run ABC algorithm with fixed vehicle configuration
            abc = MDMSMCVRP_ABC(
                problem=problem,
    ...
    if successful_runs > 0:
        return {
            ...
            'num_vehicles': 4,
            'vehicle_config': '2 per satellite',
...
def run_sensitivity_analysis(problem, best_params, n_runs_per_scenario=5):
    """Run comprehensive capacity sensitivity analysis with fixed 4 vehicles"""
    print("\n🔬 CAPACITY SENSITIVITY ANALYSIS (FIXED 4 VEHICLES)")
    print("=" * 70)
    print("Vehicle Configuration: 2 vehicles per satellite (Total: 4 vehicles)")
    print("=" * 70)

    # Define capacity variations around baseline
    satellite_capacities = [19500, 20000, 21000, 22000, 23000]  # ±12.5% around baseline
    secondary_capacities = [800, 900, 1000, 1100, 1200]

    print(f"\n📊 Satellite Capacities (W): {satellite_capacities}")
    print(f"📊 Secondary Vehicle Capacities (Q): {secondary_capacities}")
    print(f"📊 Total Scenarios: {len(satellite_capacities) * len(secondary_capacities)}")
    print(f"📊 Runs per scenario: {n_runs_per_scenario}")
    print(f"📊 Total optimization runs: {len(satellite_capacities) * len(secondary_capacities) * n_runs_per_scenario}")
    print()
...
def analyze_capacity_impact(results):
    """Analyze impact of capacity variations"""
    ...
    # Find optimal capacity combination
    optimal = min(successful_results, key=lambda x: x['mean_fitness'])

    print(f"\n🏆 OPTIMAL CAPACITY CONFIGURATION:")
    print(f"   Satellite Capacity (W): {optimal['satellite_capacity']}")
    print(f"   Secondary Vehicle Capacity (Q): {optimal['secondary_capacity']}")
    print(f"   Mean Fitness: {optimal['mean_fitness']:.2f} ± {optimal['std_fitness']:.2f}")
    print(f"   Best Fitness: {optimal['best_fitness']:.2f}")
    print(f"   Success Rate: {optimal['success_rate']:.1%}")

    # Analyze satellite capacity impact
    print(f"\n📈 SATELLITE CAPACITY (W) IMPACT:")
    satellite_caps = sorted(list(set([r['satellite_capacity'] for r in successful_results])))
    for sat_cap in satellite_caps:
        sat_results = [r for r in successful_results if r['satellite_capacity'] == sat_cap]
        mean_fitness = np.mean([r['mean_fitness'] for r in sat_results])
        print(f"   W={sat_cap}: Mean Fitness = {mean_fitness:.2f}")

    # Analyze secondary capacity impact
    print(f"\n📈 SECONDARY VEHICLE CAPACITY (Q) IMPACT:")
    secondary_caps = sorted(list(set([r['secondary_capacity'] for r in successful_results])))
    for sec_cap in secondary_caps:
        sec_results = [r for r in successful_results if r['secondary_capacity'] == sec_cap]
        mean_fitness = np.mean([r['mean_fitness'] for r in sec_results])
        print(f"   Q={sec_cap}: Mean Fitness = {mean_fitness:.2f}")


def create_sensitivity_table_data(results):
    """Create research-ready table data"""
    table_data = []

    for result in results:
        if result['successful_runs'] > 0:
            row = {
                'Satellite_Capacity_W': result['satellite_capacity'],
                'Secondary_Capacity_Q': result['secondary_capacity'],
                ...
                'Median_Fitness': f"{result['median_fitness']:.2f}",
                'Mean_Time_sec': f"{result['mean_execution_time']:.2f}"
            }
        else:
            row = {
                'Satellite_Capacity_W': result['satellite_capacity'],
                ...
                'Mean_Fitness': "Failed",
                'Std_Fitness': "Failed",
                'Median_Fitness': "Failed",
                'Mean_Time_sec': "Failed"
            }
...
def save_sensitivity_results(results, table_data):
    ...
    detailed_filename = f'capacity_sensitivity_detailed_{timestamp}.json'
    with open(detailed_filename, 'w') as f:
        json.dump({
            'experiment_info': {
                'timestamp': datetime.now().isoformat(),
                'analysis_type': 'capacity_sensitivity_analysis',
                'fixed_parameters': {
                    'num_vehicles': 4,
                    'vehicle_configuration': '2 per satellite',
                    'primary_capacity': 3000
                },
    ...
    # Save table data for LaTeX
    table_filename = f'capacity_sensitivity_table_{timestamp}.json'
    with open(table_filename, 'w') as f:
        json.dump({
            'table_headers': ['Satellite_Capacity_W', 'Secondary_Capacity_Q', 'Num_Vehicles',
                              'Success_Rate', 'Best_Fitness', 'Mean_Fitness', 'Std_Fitness',
                              'Median_Fitness', 'Mean_Time_sec'],
            'table_data': table_data
        }, f, indent=2)

    # Save CSV for easy import to Excel/LaTeX
    csv_filename = f'capacity_sensitivity_table_{timestamp}.csv'
    import csv
    ...
    if table_data and len(table_data) > 0:
        ...
        print(f"   Table CSV: {csv_filename}")
    else:
        print(f"   Table CSV: Skipped (no data)")
        csv_filename = None

    print(f"\n💾 RESULTS SAVED:")
    print(f"   Detailed JSON: {detailed_filename}")
    print(f"   Table JSON: {table_filename}")
    if csv_filename:
        print(f"   Table CSV: {csv_filename}")
...
def create_sensitivity_visualization(results):
    ...
    # Create fitness matrix for heatmap
    ...
    # 1. Heatmap: Capacity combinations
    ax1 = plt.subplot(2, 3, 1)
    im1 = ax1.imshow(fitness_matrix, cmap='RdYlGn_r', aspect='auto')
    plt.colorbar(im1, ax=ax1, label='Mean Fitness (km)')
    ax1.set_xlabel('Satellite Capacity (W)', fontsize=11)
    ax1.set_ylabel('Secondary Vehicle Capacity (Q)', fontsize=11)
    ax1.set_title('Capacity Sensitivity Heatmap', fontsize=12, fontweight='bold')
    ax1.set_xticks(range(len(satellite_caps)))
    ax1.set_xticklabels(satellite_caps, rotation=45)
    ax1.set_yticks(range(len(secondary_caps)))
    ax1.set_yticklabels(secondary_caps)

    # 2. Satellite capacity impact
    ax2 = plt.subplot(2, 3, 2)
    sat_fitness = {}
    for sat_cap in satellite_caps:
        sat_results = [r for r in successful_results if r['satellite_capacity'] == sat_cap]
        if sat_results:
            sat_fitness[sat_cap] = np.mean([r['mean_fitness'] for r in sat_results])

    ax2.plot(list(sat_fitness.keys()), list(sat_fitness.values()), 'bo-', linewidth=2, markersize=10)
    ax2.set_xlabel('Satellite Capacity (W)', fontsize=11)
    ax2.set_ylabel('Average Mean Fitness (km)', fontsize=11)
    ax2.set_title('Satellite Capacity Impact', fontsize=12, fontweight='bold')
    ax2.grid(True, alpha=0.3)

    # 3. Secondary capacity impact
    ax3 = plt.subplot(2, 3, 3)
    sec_fitness = {}
    for sec_cap in secondary_caps:
        sec_results = [r for r in successful_results if r['secondary_capacity'] == sec_cap]
        if sec_results:
            sec_fitness[sec_cap] = np.mean([r['mean_fitness'] for r in sec_results])

    ax3.plot(list(sec_fitness.keys()), list(sec_fitness.values()), 'rs-', linewidth=2, markersize=10)
    ax3.set_xlabel('Secondary Vehicle Capacity (Q)', fontsize=11)
    ax3.set_ylabel('Average Mean Fitness (km)', fontsize=11)
    ax3.set_title('Secondary Capacity Impact', fontsize=12, fontweight='bold')
    ax3.grid(True, alpha=0.3)

    # 4. Success rate distribution
    ax4 = plt.subplot(2, 3, 4)
    success_rates = [r['success_rate'] * 100 for r in results]
    ax4.hist(success_rates, bins=10, alpha=0.7, color='green', edgecolor='black')
    ax4.set_xlabel('Success Rate (%)', fontsize=11)
    ax4.set_ylabel('Frequency', fontsize=11)
    ax4.set_title('Success Rate Distribution', fontsize=12, fontweight='bold')
    ax4.axvline(np.mean(success_rates), color='red', linestyle='--', linewidth=2,
    ...
    all_fitness = [r['mean_fitness'] for r in successful_results]
    ax5.hist(all_fitness, bins=15, alpha=0.7, color='blue', edgecolor='black')
    ax5.set_xlabel('Mean Fitness (km)', fontsize=11)
    ax5.set_ylabel('Frequency', fontsize=11)
    ax5.set_title('Fitness Distribution Across All Scenarios', fontsize=12, fontweight='bold')
    ax5.axvline(np.mean(all_fitness), color='red', linestyle='--', linewidth=2,
                label=f'Mean: {np.mean(all_fitness):.1f}')
    ax5.legend()
    ax5.grid(True, alpha=0.3)

    # 6. Box plot by capacity
    ...
    for sat_cap in satellite_caps:
        sat_results = [r for r in successful_results if r['satellite_capacity'] == sat_cap]
        if sat_results:
            box_data.append([r['mean_fitness'] for r in sat_results])
            box_labels.append(sat_cap)

    ax6.boxplot(box_data, tick_labels=box_labels)
    ax6.set_xlabel('Satellite Capacity (W)', fontsize=11)
    ax6.set_ylabel('Mean Fitness (km)', fontsize=11)
    ax6.set_title('Fitness Variability by Satellite Capacity', fontsize=12, fontweight='bold')
    ax6.grid(True, alpha=0.3)

    plt.tight_layout()

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    plot_filename = f'capacity_sensitivity_analysis_{timestamp}.png'
...
def main():
    """Main function for capacity sensitivity analysis"""
    try:
        print("=" * 70)
        print("🔬 MDMS-MCVRP CAPACITY SENSITIVITY ANALYSIS")
        print("=" * 70)
        ...
        total_demand = sum(sum(demands[c].values()) for c in VC)
        ...
        print(f"\n🔧 Algorithm parameters: {best_hyperparameters}")
        ...
        # Analyze results
        analyze_capacity_impact(results)

        # Create table data
        table_data = create_sensitivity_table_data(results)

        # Save results
        detailed_file, table_file, csv_file = save_sensitivity_results(results, table_data)

        # Create visualizations
        plot_file = create_sensitivity_visualization(results)
        ...
        if successful_scenarios:
            ...
            print(f"\nFitness Results (km):")
            print(f"  • Best fitness achieved: {min(all_fitness):.2f}")
            print(f"  • Worst fitness: {max(all_fitness):.2f}")
            print(f"  • Mean fitness: {np.mean(all_fitness):.2f}")
            print(f"  • Standard deviation: {np.std(all_fitness):.2f}")
            print(f"  • Median fitness: {np.median(all_fitness):.2f}")

            # Find optimal configuration
            optimal = min(successful_scenarios, key=lambda x: x['mean_fitness'])
            print(f"\nOptimal Configuration:")
            print(f"  • Satellite capacity (W): {optimal['satellite_capacity']} units")
            print(f"  • Secondary vehicle capacity (Q): {optimal['secondary_capacity']} units")
            print(f"  • Number of vehicles: {optimal['num_vehicles']} (2 per satellite)")
            print(f"  • Mean fitness: {optimal['mean_fitness']:.2f} ± {optimal['std_fitness']:.2f} km")
            print(f"  • Best fitness: {optimal['best_fitness']:.2f} km")

        print(f"\n📁 Output Files:")
        print(f"  • Detailed results: {detailed_file}")
        print(f"  • Table (JSON): {table_file}")
        print(f"  • Table (CSV): {csv_file}")
        print(f"  • Visualization: {plot_file}")

        print("\n" + "=" * 70)
        print("✅ CAPACITY SENSITIVITY ANALYSIS COMPLETED!")
        print("=" * 70)
