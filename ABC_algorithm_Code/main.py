...
from optimization import quick_parameter_search, optimize_with_optuna
...
def print_solution_summary(solution, run_number):
    ...
    try:
        ...
        total_vehicles = len(solution.routes['E2'])
...
def extract_routes_from_solution(solution):
    """Extract route information from solution object"""
    try:
        routes_data = {
            'primary_routes': {},
            'secondary_routes': {},
            'all_routes': {},
            'num_vehicles_per_satellite': getattr(solution, 'num_vehicles_per_satellite', {})
        }

        if hasattr(solution, 'routes') and solution.routes:
            if 'E1' in solution.routes:
                routes_data['primary_routes'] = solution.routes['E1']

            if 'E2' in solution.routes:
                routes_data['secondary_routes'] = solution.routes['E2']

            routes_data['all_routes'] = solution.routes

        return routes_data
    except Exception as e:
        print(f"Warning: Could not extract routes - {e}")
        return {'primary_routes': {}, 'secondary_routes': {}, 'all_routes': {}, 'num_vehicles_per_satellite': {}}


def run_multiple_experiments(problem, best_params, num_vehicles_per_satellite=None, n_runs=30):
    """Run ABC algorithm multiple times with fixed parameters and vehicle configuration"""
    ...
    print(f"🚐 Vehicle Configuration: {num_vehicles_per_satellite} (Total: {total_vehicles} vehicles)")
    print("=" * 70)
    ...
    for run_number in range(1, n_runs + 1):
        try:
            ...
            # Initialize and run ABC with multiple vehicles
            abc = MDMSMCVRP_ABC(problem, num_vehicles_per_satellite=num_vehicles_per_satellite, **best_params)
            ...
            routes_data = extract_routes_from_solution(best_solution)

            if best_solution.fitness < best_overall_fitness:
                best_overall_fitness = best_solution.fitness
    ...
    if successful_runs > 0:
        summary_stats = {
            ...
            'fitness_values_list': [float(f) for f in fitness_values]
        }

        best_run_index = fitness_values.index(min(fitness_values))
        best_run = all_results[best_run_index]

        print(f"📊 EXPERIMENT SUMMARY:")
        print(f"   Successful runs: {successful_runs}/{n_runs} ({summary_stats['success_rate']:.1%})")
        print(f"   Best fitness: {summary_stats['best_fitness']:.4f} (Run #{best_run['run_number']})")
        ...
        print(f"   Vehicles: {total_vehicles} ({num_vehicles_per_satellite})")
        ...
        final_results = {
            ...
            'best_solution': {
                ...
                'routes': best_run['routes'],
                'customers_served': best_run['customers_served'],
                'execution_time': best_run['execution_time_seconds'],
...
def create_results_visualization(fitness_values, convergence_data=None, num_vehicles=None, save_plots=True):
    ...
    title = f'Fitness Values Across Runs'
    if num_vehicles:
        title += f' ({num_vehicles} vehicles)'
    ...
    if save_plots:
        ...
        if num_vehicles:
            plot_filename = f'experiment_results_{num_vehicles}vehicles_{timestamp}.png'
...
def run_hyperparameter_optimization(problem, num_vehicles_per_satellite=None, use_quick_search=True):
    """Run hyperparameter optimization using optimization.py"""
    ...
    if use_quick_search:
        print("Using QUICK parameter search...")
        best_params = quick_parameter_search(problem, num_vehicles_per_satellite, save_results=True)
    else:
...
def main():
    """Main function with hyperparameter optimization and multiple runs support"""
    try:
        print("🔬 MDMS-MCVRP Experiment Framework")
        print("=" * 70)
        ...
        # CONFIGURE VEHICLE NUMBERS HERE - EASY TO CHANGE
        num_vehicles_per_satellite = {
            'S1': 2,  # 2 vehicles from satellite S1
            'S2': 2  # 2 vehicles from satellite S2
        }
        # Change these numbers to experiment with different configurations!
        ...
        # CHOOSE YOUR MODE
        mode = input("\n🎯 Choose mode:\n"
                     "  1 - Run with predefined best parameters\n"
                     "  2 - Run quick parameter search first, then experiments\n"
                     "  3 - Run full Optuna optimization, then experiments\n"
                     "Enter choice (1/2/3): ").strip()

        if mode == "2":
            # Quick parameter search
            best_hyperparameters = run_hyperparameter_optimization(
                problem, num_vehicles_per_satellite, use_quick_search=True)
            if not best_hyperparameters:
                print("❌ Hyperparameter optimization failed. Using defaults.")
            ...
            best_hyperparameters = run_hyperparameter_optimization(
                problem, num_vehicles_per_satellite, use_quick_search=False)
            if not best_hyperparameters:
                print("❌ Hyperparameter optimization failed. Using defaults.")
                best_hyperparameters = {
                    'swarm_size': 80,
        ...
        try:
            n_runs = int(input("\n🔢 Enter number of experimental runs (default 30): ").strip() or "30")
        except ValueError:
            n_runs = 30

        # Run experiments
        print(f"\n🎯 Running {n_runs} experiments with selected hyperparameters...")
        ...
        if results and fitness_values:
            # Save results
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            results_filename = f'{n_runs}_runs_{total_vehicles}vehicles_{timestamp}.json'
            ...
            if save_results_to_json(results, results_filename):
                ...
                print(f"Fitness values: {[f'{f:.4f}' for f in fitness_list[:10]]}...")  # First 10
                print(f"\nSummary Statistics:")
                print(f"  Best Fitness: {results['summary_statistics']['best_fitness']:.4f}")
                print(f"  Mean Fitness: {results['summary_statistics']['mean_fitness']:.4f}")
                print(f"  Std Deviation: {results['summary_statistics']['std_fitness']:.4f}")
                print(f"  Median Fitness: {results['summary_statistics']['median_fitness']:.4f}")

                print(f"\n🏆 BEST SOLUTION (Run #{results['best_solution']['run_number']}):")
                print(f"   Fitness: {results['best_solution']['fitness']:.4f}")
                print(f"   Customers Served: {results['best_solution']['customers_served']}")
                print(f"   Vehicles Used: {results['best_solution']['num_vehicles']}")
                print(f"   Routes stored in JSON file")

                # Create visualizations
                last_convergence = results['individual_runs'][-1]['convergence_curve'] if results[
                    'individual_runs'] else None
                create_results_visualization(fitness_values, last_convergence, total_vehicles, save_plots=True)

                print(f"\n✅ Experiment completed successfully!")
                print(f"📁 Check '{results_filename}' for detailed results")
                print(f"🗺️  Use 'best_solution' → 'routes' section for map visualization")
